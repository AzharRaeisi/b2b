...
// ADD / EDIT FORM SUBMIT FOR DATA TABLE
        ...
        $(document).on('submit','#yiwuformdata',function(e){
            e.preventDefault();
            if(admin_loader == 1)
            {
                $('.submit-loader').show();
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    if ((data.errors)) {
                        ...
                        $('#yiwuformdata input , #yiwuformdata select , #yiwuformdata textarea').eq(1).focus();
                    }
                    else
                    {
                        table.ajax.reload();
...
// NORMAL FORM
    ...
    $(document).on('submit','#yiwuform',function(e){
        e.preventDefault();
        if(admin_loader == 1)
        {
            $('.gocover').show();
        ...
        $.ajax({
            ...
            success:function(data)
            {
                //console.log(data);
                if ((data.errors)) {
                    yiwuform.parent().find('.alert-success').hide();
                    yiwuform.parent().find('.alert-danger').show();
                    yiwuform.parent().find('.alert-danger ul').html('');
                    for(var error in data.errors)
                    {
                        $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
                    }
                    yiwuform.find('input , select , textarea').eq(1).focus();
                }
                else
                {
                    yiwuform.parent().find('.alert-danger').hide();
                    yiwuform.parent().find('.alert-success').show();
                    yiwuform.parent().find('.alert-success p').html(data);
                    yiwuform.find('input , select , textarea').eq(1).focus();
                }
                if(admin_loader == 1){
                    $('.gocover').hide();
...
// SEND MESSAGE SECTION
    ...
    $(document).on("submit", "#emailreply1" , function(){
        ...
        $.ajax({
            type: 'post',
            url: mainurl+'/yiwu-bazar-mgmt/user/send/message',
            data: {
                '_token': token,
                'subject'   : subject,
                'message'  : message,
...
// SEND EMAIL SECTION
    ...
    $(document).on("submit", "#emailreply" , function(){
        ...
        $.ajax({
            type: 'post',
            url: mainurl+'/yiwu-bazar-mgmt/order/email',
            data: {
                '_token': token,
                'subject'   : subject,
                'message'  : message,
