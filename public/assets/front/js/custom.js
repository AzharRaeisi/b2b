            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if($.trim(result.success) == 'true'){
                        ...
                        $this.find('.alert-danger').hide();
                        $this.find('.alert-success').show();
                        $this.find('.alert-success p').html(result.message);
                        ...
                        $this.find('.alert-danger').show();
                        $this.find('.alert-danger ul').html('');
                        $this.find('.alert-danger p').html(errorsShow);
                    }
                    if(typeof (result.send_to) != 'undefined'){
                        window.location.href = result.send_to;
                    }
...
// Adding Muliple Quantity Starts
        ...
        $(document).on('click', '.product-size .siz-list .box', function () {
            ...
            if(pos == '0'){
                updatePrice(sign+total);
            }else {
                updatePrice(total+sign);
            }

        });
        $(document).on('click', '.product-size .siz-list .box-login', function () {
...
// CANCEL CLICK ENDS
        ...
        /*-----------------------------
         Cart Page Quantity
         -----------------------------*/
        $(document).on('click', '.qttotal', function () {
            var el = $(this);
            el.hide();
            $('#edit_qty_text').show();
            $('#edit_qty_text').focus();
            $('#edit_qty_text').val(el.text());
        });
        $(document).on('blur', '#edit_qty_text', function () {
            var el = $(this);

            var el_min = $('.qtminus');
            var min_qty = parseInt(el_min.attr('data-min'));

            el.hide();
            $('.qttotal').show();
            if(min_qty > el.val()){
                toastr.error('Minimum quantity should be '+min_qty);
                return false;
            }
            $('.qttotal').text(el.val());
        ...
        $(document).on('click', '.qtminus', function () {
            var el = $(this);
            var min_qty = parseInt(el.attr('data-min'));
            ...
            //alert(min_qty+'==>'+total);
            if (total > 1) {
                if($.trim(min_qty) != '' && min_qty > 0 && total > min_qty){
                    total--;
                }
            }
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(data == 'digital') {
                        ...
                        $("#cart-count").html(data[0]);
                        $("#cart-items").load(mainurl+'/user/carts/view');
                        toastr.success(langg.add_cart);
                    }
                }
            });
...
// TRACK ORDER ENDS
    ...
    function updatePrice(price_value){
        $('#sizeprice').hide().html(price_value).show('slow');
    }
});
