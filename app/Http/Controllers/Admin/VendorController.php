...
class VendorController extends Controller
{
    ...
    {
        $datas = User::whereIn('is_vendor',['1','2'])->orderBy('id','desc')->get();
        ...
        return Datatables::of($datas)
            ->addColumn('status', function(User $data) {
                ...
                return '<div class="action-list"><select class="process select vendor-droplinks '.$class.'">'.
                    '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>Activated</option>'.
                    '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Deactivated</option></select></div><a href="javascript:;" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->verifies->id.'"><i class="fas fa-paperclip"></i> View Attachments</a>';
            })
            /*->addColumn('status', function(User $data) {
                $class = $data->status == 1 ? 'drop-success' : 'drop-danger';
    ...
    public function verifySubmit(Request $request, $id)
    {
        ...
        $verification->admin_warning = 1;
        $verification->warning_reason = $request->details;
        $text = json_decode($verification->text,1);
        $text[] = 'Admin required more verification ('.$request->details.').';
        $verification->text = json_encode($text);
        $verification->save();
