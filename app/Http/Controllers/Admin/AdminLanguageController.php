...
class AdminLanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        file_put_contents(base_path().'/resources/lang/'.$data->file, $mydata);
        //--- Logic Section Ends
    ...
    public function edit($id)
    {
        ...
        $data_results = file_get_contents(base_path().'/resources/lang/'.$data->file);
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
        $data = AdminLanguage::findOrFail($id);
        if (file_exists(base_path().'/resources/lang/'.$data->file)) {
            unlink(base_path().'/resources/lang/'.$data->file);
        }
        $data->language = $input['language'];
        $name = time().str_random(8);
        $data->name = $name;
