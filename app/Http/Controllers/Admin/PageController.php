...
class PageController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('footer', function(Page $data) {
                ...
                $s3 = '';
                $ns = '';
                if($data->footer_menu_1 == '1'){
                    $s1 = 'selected';
                    $class = 'drop-success';
                }else if($data->footer_menu_2 == '1'){
                    $s2 = 'selected';
                    $class = 'drop-success';
                }else if($data->footer_menu_3 == '1'){
                    $s3 = 'selected';
                    $class = 'drop-success';
                }else{
                    $ns = 'selected';
                ...
                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('admin-page-footer',['id1' => $data->id, 'id2' => 1]).'" '.$s1.'>Showed in menu 1</option><option data-val="1" value="'. route('admin-page-footer',['id1' => $data->id, 'id2' => 2]).'" '.$s2.'>Showed in menu 2</option><option data-val="1" value="'. route('admin-page-footer',['id1' => $data->id, 'id2' => 3]).'" '.$s3.'>Showed in menu 3</option><option data-val="0" value="'. route('admin-page-footer',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Not Showed</option>/select></div>';
            })
            ...
            })->rawColumns(['header','footer','action'])->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    public function store(Request $request)
    {
        //--- Validation Section
        $slug = $request->slug;
        $main = array('home','blog','cart','checkout');
    ...
    {
        $data = Page::findOrFail($id1);
        $data->footer_menu_1 = '0';
        $data->footer_menu_2 = '0';
        $data->footer_menu_3 = '0';
        if($id2 == '1'){
            $data->footer_menu_1 = '1';
        }
        if($id2 == '2'){
            $data->footer_menu_2 = '1';
        }
        if($id2 == '3'){
            $data->footer_menu_3 = '1';
        }
        $data->update();
    }
