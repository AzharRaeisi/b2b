...
class UserController extends Controller
{
    ...
    {
        $datas = User::where('is_vendor','=','0')->orderBy('id','DESC')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->addColumn('status', function(User $data) {
                $class = $data->status == 1 ? 'drop-success' : 'drop-danger';
                $status = ($data->status == 1 ? 'Activate' : 'In-Activate');
                return '<div class="action-list"><a href="javascript:;" class="'.$class.'">'.$status.'</a></div><a href="javascript:;" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->verifies->id.'"><i class="fas fa-paperclip"></i> View Attachments</a>';
            })
            ->addColumn('action', function(User $data) {
                ...
                $ban = '<select class="process select droplinks '.$class.'">'.
                    '<option data-val="1" value="'. route('admin-user-ban',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>Activated</option>'.
                    '<option data-val="0" value="'. route('admin-user-ban',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Deactivated</option></select>';
                $ban= '';//Add becuase status is manageing wiith verificatio
                return '<div class="action-list"><a href="' . route('admin-user-show',$data->id) . '" > <i class="fas fa-eye"></i> Details</a><a data-href="' . route('admin-user-edit',$data->id) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i>Edit</a><a href="javascript:;" class="send" data-email="'. $data->email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> Send</a>'.$ban.'<a href="javascript:;" data-href="' . route('admin-vendor-verify',$data->id) . '" class="verify" data-toggle="modal" data-target="#verify-modal"> <i class="fas fa-question"></i> Ask For Verification</a><a href="javascript:;" data-href="' . route('admin-user-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
