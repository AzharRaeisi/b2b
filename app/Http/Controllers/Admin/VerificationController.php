...
class VerificationController extends Controller
{
    ...
    public function datatables($status)
    {
        ...
        $datas = $datas->where('type','=','Vendor');
        return Datatables::of($datas)
            ->addColumn('name', function(Verification $data) {
                $name = isset($data->user->name) ? '<a href="'. route('admin-vendor-show',$data->user->id) .'" target="_blank">'.$data->user->name.'</a>' : 'Removed';
                return  $name;
            })
            ...
            ->editColumn('text', function(Verification $data) {
                $details = '<p>'.implode('<br>',json_decode($data->text,1)).'</p>';
                return  $details;
            })
    ...
    {
        return view('admin.verify.buyer-index');
    }
    //*** JSON Request
    public function bvDatatables($status)
    {
        if($status == 'Pending'){
            $datas = Verification::where('status','=','Pending')->get();
        }
        ...
        return Datatables::of($datas)
            ...
            ->editColumn('address', function(Verification $data) {
                $details = $data->user->address.' '.$data->user->city.', '.$data->user->country;
                return  $details;
            })
            ->addColumn('status', function(Verification $data) {
                $class = $data->status == 'Pending' ? '' : ($data->status == 'Verified' ? 'drop-success' : 'drop-danger');
            ...
            ->addColumn('action', function(Verification $data) {
                return '<div class="action-list"><a href="javascript:;" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-paperclip"></i> View Attachments</a><a href="javascript:;" data-href="' . route('admin-vr-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a><br><a target="_blank" href="'.route('admin-user-show',$data->user_id).'"><i class="fas fa-user"></i> View Buyer Details</a></div>';
            })
            ->rawColumns(['name','status','action','text'])
            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    {
        return view('admin.verify.buyer-pending');
    }
    ...
    public function show()
    {
        ...
        if(count($prod))
        {
            $data[0] = 1;
            $data[1] = $prod;
            $data[2] = implode('<br>',json_decode($prod1->text,1));
    ...
    public function status($id1,$id2)
    {
        ...
        $user->status = $id2;
        $text = json_decode($user->text,1);
        $text[] = 'Status has been changed to '.$id2;
        $user->text = json_encode($text);
        $user->update();
