...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('type', function(Product $data) {
    ...
    {
        // $cats = Category::whereIn('type',['product','services','both'])->get();
    ...
    {
        $cats = Category::whereIn('type',['services','both'])->get();
        $sign = Currency::where('is_default','=',1)->first();
    ...
    public function store(Request $request)
    {
        ...
        if($request->type == "Physical")
        {
            ...
            //--- Validation Section
            $rules = ['sku'      => 'min:5|unique:products'];
            ...
            // if ($request->minimum_quantity_check == "")
            // {
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function edit($id)
    {
        ...
        if($data->type == 'Digital'){
            ...
            $cats = $cats->whereIn('type',['product','services','both']);
        }
        $cats = $cats->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data->type == "Physical")
        {
            ...
            //--- Validation Section
            $rules = ['sku' => 'min:5|unique:products,sku,'.$id];
    ...
    public function featuresubmit(Request $request, $id)
    {
        ...
        if(!empty($input['discount_date'])){
            $input['discount_date'] = date('Y-m-d',strtotime($input['discount_date']));
        }
...
// PRODUCT DELETE ENDS
    ...
    function duplicate($id){
        ...
        if($data->type == 'Digital'){
            ...
            $cats = $cats->whereIn('type',['product','both']);
        }
        $cats = $cats->get();
        ...
        if($data->type == 'Digital')
            return view('admin.product.duplicate.digital',compact('cats','data','sign'));
        else
            return view('admin.product.duplicate.physical',compact('cats','data','sign'));
    }
    public function duplicatePost(Request $request, $id)
    {
        //debug($request->all(),1);
        //--- Validation Section
        $rules = [
        ...
        if ($files = $request->get('duplicate-gallery-images')) {
            foreach ($files as $file) {
                    $gallery = new Gallery;
                    $name = 'dp-'.$file;
                    \File::copy(public_path('assets/images/galleries/'.$file), public_path('assets/images/galleries/'.$name));
                    $gallery['photo'] = $name;
                    $gallery['product_id'] = $lastid;
                    $gallery->save();
            }
