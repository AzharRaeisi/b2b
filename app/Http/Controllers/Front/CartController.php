...
class CartController extends Controller
{
    ...
    public function addnumcart()
    {
        ...
        $product_qty_price = calculatePriceByQty($prod->price, $prod->whole_sell_qty,$prod->whole_sell_discount,$qty);

        if($prod->user_id != 0){
            $gs = Generalsetting::findOrFail(1);
            $prc = $product_qty_price + $gs->fixed_commission + ($product_qty_price/100) * $gs->percentage_commission ;
            $product_qty_price = round($prc,2);
        }
        if(!empty($prices))
        {
            foreach($prices as $data){
                $product_qty_price += ($data / $curr->value);
            }
