...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null){
        ...
        $prods = $prods->where('status', '=', '1')->where('type', '=','Physical')->paginate(15);
        //debug($prods,1);
        $data['prods'] = $prods;
        if($request->ajax()) {
            $data['ajax_check'] = 1;
    ...
    public function service(Request $request, $slug=null, $slug1=null, $slug2=null){
        if (Session::has('currency'))
        {
            $curr = Currency::find(Session::get('currency'));
        }
        ...
        $prods = $prods->where('status', 1)->where('type', '=','Digital')->get();
        $prods = (new Collection(Product::filterProducts($prods)))->paginate(15);
        $data['prods'] = $prods;
        if($request->ajax()) {
            $data['ajax_check'] = 1;
            return view('includes.product.filtered-products', $data);
...
//        $product->update();
    ...
    function setSearch(Request $request){
        if($request->product_type == 'service'){
            $route = route('front.service').'?search='.$request->search;
        }elseif ($request->product_type == 'suppliers'){
            $route = route('front.vendorStores').'?search='.$request->search;
        }else{
            $route = route('front.category').'?search='.$request->search;
        }
        if(!empty($request->search)){
            SearchData::where('search_text', '=' ,$request->search)->where('ip_address','=',getRealIpAddr())->delete();
            SearchData::insert(['search_type'=>$request->product_type,'ip_address'=>getRealIpAddr(),'search_text'=>$request->search]);
        }
