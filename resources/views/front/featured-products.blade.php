                        ...
                        <ul class="all_prod_cats">
                            ...
                            @foreach($categories as $category)
                                <li class="{{count($category->subs) > 0 ? 'dropdown_list':''}} {{ $i >= 15 ? 'rx-child' : '' }}"> @if(count($category->subs) > 0)
                                        ...
                                        <div class="link-area">@if(count($category->subs) > 0) <a href="javascript:;"> <i class="fa fa-angle-down" aria-hidden="true"></i> </a> @endif <a href="{{ route('front.category',$category->slug) }}">{{ $category->name }}</a></div>
                                    ...
                                    @if(count($category->subs) > 0)
                                        ...
                                        <ul class="{{ $ck == 1 ? 'categories_mega_menu' : 'categories_mega_menu column_1' }}">
                                            @foreach($category->subs as $subcat)
                                                <li> <a href="{{ route('front.subcat',['slug1' => $subcat->category->slug, 'slug2' => $subcat->slug]) }}">@if(count($subcat->childs) > 0) <i class="fa fa-angle-down"></i> @endif{{$subcat->name}}</a> @if(count($subcat->childs) > 0)
                ...
                <div class="all_prod_tab">
                    <div class="all_prod_tab_main">
                        <ul class="nav nav-tabs">
                            ...
                            <li><a href="{{route('front.featuredCatproducts')}}" class="active">Products</a></li>
                    ...
                    <div class="tab-content">
                        <div class="text-center mt-3"><a style="color: #af241e" href="{{route('front.category')}}"><strong>View all Products</strong></a></div>
                        <div id="suppliers" class="tab-pane fade in active show">
                            <div class="all_prod_cats_main">
                                @foreach($category_selections as $cat)
                                    ...
                                    <div class="prod_botm_btn"><a href="{{route('front.category',$cat->slug)}}">View more</a></div>
                                @endforeach
                            </div>
                        </div>
